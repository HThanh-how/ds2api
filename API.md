...
- 若已在配置中启用 Turso（`turso.url` 与 token），业务 API key 会以哈希形式镜像到 Turso；校验时**优先**查库，查询失败或库中无记录时回退到本地 `config.keys` / `api_keys`。**管理员仍通过配置文件或 Admin 接口维护 key**；配置保存成功后会将全量 key 集合同步到 Turso（无 Turso 时行为与原先仅本地校验一致）。
